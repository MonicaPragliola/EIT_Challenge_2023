...
The main code main_IAS.m takes as input the input folder containing the data, the output folder and the difficulty levels (DL), that encodes the downsampling factor in the set of injection patterns. In example.m you can find a call of main_IAS.m for the training data provided by the KTC2023 organizers. The auxiliary functions and files have to be stored in the same folder as main_IAS.m.
